...
def updatePef_rds(credentials, instanceTypeDict):
    ...
    for dbInstance in dbInstances:
        ...
        performs.append(CustomPerfDO(
            ...
            cpuAvg=cpuUtil[0] if cpuUtil[0] != 0 else None,
            cpuMax=cpuUtil[1] if cpuUtil[1] != 0 else None,
            memAvg=round((dbInstanceMem-freeMem[0])/dbInstanceMem*100,2) if freeMem[0] != 0 else None,
            memMax=round((dbInstanceMem-freeMem[2])/dbInstanceMem*100,2) if freeMem[2] != 0 else None,
            disk_total=None,
            disk_used=None,
            disk_used_pct=None))
