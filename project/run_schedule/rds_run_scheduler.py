...
def main():
    ...
    """ 2) RDS 목록 가지고 오기 """
    # rds 인스턴스 목록
    rds_list = describe_db_instances(rds_client)
    ...
    for rds in rds_list:
        ...
        """ 3-2) 호출한 시간 비교 및 실행 """
        run_schedule(rds_client, rds.get('DBInstanceIdentifier'), holiday_list, sch_except, sch_start, sch_stop, cur_datetime)
...
2) RDS 목록 가지고 오기
"""
def describe_db_instances(client):
    return client.describe_db_instances()


"""
3) RDS에 대한 태그를 비교하여 Start/Stop 동작
"""
def run_schedule(client, db_instance_id, holiday_list, sch_except=None, sch_start=None, sch_stop=None, cur_datetime=None): 
