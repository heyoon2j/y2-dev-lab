...
def main():
    ...
    # 1-3)
    service = getattr(cloudev, 'EC2')
    ec2_client = service(region_name=region_name, iam_role_arn=iam_role_arn, aws_access_key_id=aws_access_key_id, aws_secret_access_key=aws_secret_access_key, profile_name=profile_name)
    
    """ 2) 인스턴스 목록 가지고 오기 """ 
    instance_list = describe_ec2_instances(ec2_client)
    ...
    for instance in instance_list:
        ...
        """ 3-2) 호출한 시간 비교 및 실행 """
        run_schedule(ec2_client, instance.get('InstanceId'), holiday_list, sch_except, sch_start, sch_stop, cur_datetime)
...
3) 인스턴스에 대한 태그를 비교하여 Start/Stop 동작
"""
def run_schedule(client, instance_id, holiday_list, sch_except=None, sch_start=None, sch_stop=None, cur_datetime=None): 
# 3-2) 호출한 시간 비교 
    # - RUN_SCHEDULE : daily / daily_period / nonStop_period / stop_period
    # - RUN_SCH_TIMESTAMP
    #     - 매일 : 10:00_20:00
    ...
    if sch_except:
        ...
        if start_datetime.date() == cur_datetime.date() and start_datetime.time().hour == cur_time.hour and start_datetime.time().minute:
            ...
            client.stop_instances(instance_id)
            return
        else:
            return
