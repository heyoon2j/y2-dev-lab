...
def main():
    ...
    # 1-3)
    service = getattr(cloudev, 'rds')
    rds_client = service(region_name=region_name, iam_role_arn=iam_role_arn, aws_access_key_id=aws_access_key_id, aws_secret_access_key=aws_secret_access_key, profile_name=profile_name)
    
    service = getattr(cloudev, 'Aurora')
    aurora_client = service(region_name=region_name, iam_role_arn=iam_role_arn, aws_access_key_id=aws_access_key_id, aws_secret_access_key=aws_secret_access_key, profile_name=profile_name)
    

    """ 2) Aurora Cluster 목록 가지고 오기 """
    # aurora 클러스터 목록
    aurora_list = describe_db_clusters(aurora_client)


    """ 3) 태그를 비교하여 Start/Stop 동작 """
    for aurora in aurora_list:
        """ 3-1) 태그 확인 """
        tags = dict()
        for tag in aurora.get('TagList',[]):
            tags[tag['Key']] = tag['Value']

        sch_except = tags.get('RUN_SCH_EXCEPT')
        sch_start = tags.get('RUN_SCH_START')
        sch_stop = tags.get('RUN_SCH_STOP')

        """ 3-2) 호출한 시간 비교 및 실행 """
        run_schedule(aurora_client, aurora.get('DBClusterIdentifier'), holiday_list, sch_except, sch_start, sch_stop, cur_datetime)
...
def get_holidays():
    ...
    return list(kr_holidays.key())


"""
2) Aurora 클러스터 목록 가지고 오기
"""
def describe_db_clusters(client):
    return client.describe_db_clusters()


"""
3) Aurora 클러스터에 대한 태그를 비교하여 Start/Stop 동작
"""
def run_schedule(client, db_cluster_id, holiday_list, sch_except=None, sch_start=None, sch_stop=None, cur_datetime=None): 
    ...
    if sch_except:
        ...
        if start_datetime.date() == cur_datetime.date() and start_datetime.time().hour == cur_time.hour and start_datetime.time().minute:
            client.start_db_cluster(DBClusterIdentifier=db_cluster_id)
            return
            ...
            client.stop_db_cluster(DBClusterIdentifier=db_cluster_id)
            return
        else:
            return
